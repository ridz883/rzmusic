const App={
    init(){
        ...
        setTimeout(function(){ 
            App.checkUrl();
            // Load lagu terakhir jika tidak ada URL play
            var p=new URLSearchParams(location.search);
            if(!p.get('play')&&!p.get('sharedpl')){
                loadLastTrack();
            }
        }, 1000);
    },
...
const Library={
    ...
    render(){
        ...
        '<button onclick="Library.switchTab(\'playlist\')" class="px-4 py-2 rounded-full text-sm font-medium transition-all '+(tab==='playlist'?'bg-white text-black':'glass text-white')+'">Playlist</button>'+
        '<button onclick="Library.switchTab(\'favorite\')" class="px-4 py-2 rounded-full text-sm font-medium transition-all '+(tab==='favorite'?'bg-white text-black':'glass text-white')+'">Favorit ❤️</button>'+
        '<button onclick="Library.switchTab(\'history\')" class="px-4 py-2 rounded-full text-sm font-medium transition-all '+(tab==='history'?'bg-white text-black':'glass text-white')+'">Riwayat</button>'+
        '</div>';
    ...
    _renderPlaylist(){
        ...
        pls.forEach(function(p){
            ...
            '<img src="'+(p.image||FI)+'" onclick="Library.open(\''+p.id+'\')" class="w-full aspect-square object-cover rounded-lg mb-2" loading="lazy" />'+
    ...
    _renderFavorite(){
        ...
        favs.forEach(function(t,i){
            ...
            '<button onclick="event.stopPropagation();toggleFavorite();Library.render();" class="text-red-400 p-2 flex-shrink-0"><i data-lucide="heart" class="w-4 h-4 fill-current"></i></button>'+
            '</div>';
        });
        return html+'</div>';
    },
