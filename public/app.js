const App={
    init(){
        ...
        <div class="pt-12 px-4 text-center">
            ...
            <h1 class="text-3xl font-black chrome-text mb-1">NanzzMusify</h1>
            ...
            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-3 text-left mb-4">
                ...
                <div class="flex justify-between"><span class="text-[#6b7280] text-sm">Nama</span><span class="text-white font-medium text-sm">NanzzMusify</span></div>
            ...
            <div class="glass rounded-2xl p-5 max-w-sm mx-auto space-y-3 text-left mb-6">
                <h3 class="text-[#cfd3d8] font-bold text-sm uppercase tracking-wider mb-2 flex items-center gap-1.5"><i data-lucide="user" class="w-3.5 h-3.5"></i>Developer</h3>
                <div class="flex justify-between"><span class="text-[#6b7280] text-sm">Nama</span><span class="text-white font-bold text-sm">Nanzz</span></div>
                <div class="flex justify-between"><span class="text-[#6b7280] text-sm">TikTok</span><span class="text-[#cfd3d8] font-bold text-sm">@nanasajaudah</span></div>
            </div>
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='NanzzMusify',cover=FI,artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'NanzzMusify',cover:FI,artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            setTimeout(function(){FullPlayer.open();loadTrack(S.ct);},400);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='NanzzMusify',cover=FI;
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','NanzzMusify',FI);});
    },
