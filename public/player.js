...
var DynBG={
  ...
  update:function(coverUrl){
    ...
    img.onload=function(){
      try{
        ...
        for(var i=0;i<d.length;i+=4){
          ...
          if(bright>20&&bright<220){r+=d[i];g+=d[i+1];b+=d[i+2];count++;}
        }
        if(count>0){r=Math.floor(r/count);g=Math.floor(g/count);b=Math.floor(b/count);}
        // Gelapkan warna supaya tetap dark theme
        r=Math.floor(r*0.25);g=Math.floor(g*0.25);b=Math.floor(b*0.25);
        var grad='radial-gradient(ellipse at top, rgba('+r+','+g+','+b+',0.7) 0%, #050507 65%)';
    ...
    img.onerror=function(){document.body.style.background='#050507';};
    img.src=coverUrl;
  }
};
