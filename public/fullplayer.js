const FullPlayer={
    init(){
        ...
        <div id="full-player" class="fixed flex flex-col z-[100]" style="display:none;background:rgba(5,5,7,0.92);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);transition:transform 0.35s ease-out;transform:translateY(100%);top:0;left:0;width:100%;height:100%;overflow:hidden;touch-action:none;">
            <div class="absolute inset-0 z-0 pointer-events-none"><img id="full-bg-blur" src="" class="w-full h-full object-cover blur-[48px] opacity-35" /><div class="absolute inset-0 bg-gradient-to-b from-black/40 via-black/10 to-[#050507]"></div></div>
            ...
            <div class="relative z-10 flex-1 flex items-center justify-center px-8" style="min-height:0;overflow:hidden;">
                <div class="relative w-full max-w-[280px] aspect-square"><img id="full-cover" src="" class="w-full h-full object-cover shadow-2xl shadow-black/50 transition-transform duration-500 vinyl-paused" style="border-radius:50%;box-shadow:0 0 0 8px rgba(255,255,255,0.05),0 20px 60px rgba(0,0,0,0.8);" /></div>
            </div>
            <div class="relative z-10 px-6 pb-4 flex-shrink-0">
                <div class="flex justify-between items-start mb-3">
                <div class="flex-1 truncate">
                    <p id="full-lyric-preview" class="text-white/60 text-xs mb-1 truncate italic hidden"></p>
                    <h2 id="full-title" class="text-lg font-bold text-white truncate">Pilih lagu</h2>
                    <p id="full-artist" class="text-[#b3b3b3] text-sm truncate cursor-pointer hover:text-[#cfd3d8]" onclick="FullPlayer.openArtist()"></p>
                </div>
                    <div class="flex gap-1">
                        <button id="fav-btn" onclick="toggleFavorite()" class="text-white/60 hover:text-white active:scale-90 p-2 transition" title="Favorit"><i data-lucide="heart" class="w-5 h-5"></i></button>
                        ...
                        <button id="radio-btn" onclick="toggleRadioMode()" class="text-white/60 hover:text-white active:scale-90 p-2 transition" title="Radio Mode"><i data-lucide="radio" class="w-5 h-5"></i></button>
                        <button id="smart-queue-btn" onclick="toggleSmartQueue()" class="text-white/60 hover:text-white active:scale-90 p-2 transition" title="Smart Queue"><i data-lucide="sparkles" class="w-5 h-5"></i></button>
                        ...
                        <button onclick="shareToStory()" class="text-white/60 hover:text-white active:scale-90 p-2 transition" title="Share ke Story"><i data-lucide="image" class="w-5 h-5"></i></button>
                    </div>
                </div>
        ...
        <div id="lyrics-overlay" class="fixed flex flex-col z-[250]" style="display:none;background:rgba(5,5,7,0.88);backdrop-filter:blur(40px);-webkit-backdrop-filter:blur(40px);transition:transform 0.35s ease-out;transform:translateY(100%);top:0;left:0;width:100%;height:100%;overflow:hidden;touch-action:none;">
            <div class="flex justify-between items-center p-4 pt-6 flex-shrink-0 relative z-10">
            ...
            <div class="flex-1 overflow-y-auto px-8 py-8 hide-scrollbar relative z-10"><div id="lyrics-loading" class="flex justify-center items-center h-full"><div class="w-8 h-8 border-3 border-[#cfd3d8] border-t-transparent rounded-full animate-spin"></div></div><div id="lyrics-content" class="hidden pb-48 pt-16"></div><div id="lyrics-empty" class="hidden flex justify-center items-center h-full text-[#6b7280]"><div><i data-lucide="music" class="w-16 h-16 mx-auto mb-4 opacity-30"></i><p>Lirik tidak tersedia</p></div></div></div>
        </div>`;
        lucide.createIcons();
    },
    open(){
        var fp=gid('full-player');
        fp.classList.add('fp-animating'); // matiin blur dulu selama slide masuk biar gak nge-lag
        fp.style.display='flex';document.body.style.overflow='hidden';document.body.style.position='fixed';document.body.style.width='100%';
        requestAnimationFrame(function(){fp.style.transform='translateY(0)';});
        setTimeout(function(){fp.classList.remove('fp-animating');},380); // blur asli nyala lagi pas udah diem
        MP.hide();
    },
    close(){
        var fp=gid('full-player');
        fp.classList.add('fp-animating');
        fp.style.transform='translateY(100%)';document.body.style.overflow='';document.body.style.position='';document.body.style.width='';
        setTimeout(function(){fp.style.display='none';fp.classList.remove('fp-animating');MP.show();},350);
    },
